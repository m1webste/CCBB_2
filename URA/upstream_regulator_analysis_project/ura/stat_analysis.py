...
def tr_pvalues(DG, background_list, DEG_list):
    ...
    for TR in source_nodes:
        x = len(
            list(set(DG.neighbors(TR)) & set(DEG_list)))  # per TR, observed overlap between TR neighbors and DEG_list
        ...
        N = len(list(set(background_list) & set(
            DEG_list)))  # number of DEG, picked from universe "at random" (TODO: intersection)
